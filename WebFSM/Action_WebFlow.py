...
#添加的部分
class NodeData:
    def __init__(self, action_list, connection_node_dict):
        ''' action_list根据action顺序排，结构为 {"click" : [response_list], click" : [request_list], "input" : [response_list], "input" : [request_list], ...} '''
        self.action_list = action_list
        ''' connection_node根据顺序排，结构为: {"webpage_Z(名字会是webpage对应的url)" = ["click" : [request_list] 或者 "input" : [request_list]], ...} '''
        self.connection_node = connection_node_dict

class ListNode:
    def __init__(self, webpage_name):
        self.webpage_name = webpage_name
        # 存储各个网页拥有的action和connection结点
        self.data_list = []
        self.next = None

    def data_list_append(self, node_data):
        self.data_list.append(node_data)

class LinkedList:
    def __init__(self):
        self.head = None
        #存储各个role拥有的网页结点
        self.role_group_list = {}

    def append_node(self, role_group, webpage_name, node_data):
        if not self.head:
            self.head = ListNode(webpage_name)
            self.head.data_list_append(node_data)
            if role_group not in self.role_group_list:
                self.role_group_list[role_group] = []
            self.role_group_list[role_group].append(self.head)

        else:
            current = self.head
            while current.next:
                current = current.next
            current.next = ListNode(node_data)
            if role_group not in self.role_group_list:
                self.role_group_list[role_group] = []
            self.head = ListNode(role_group)
            self.head.data_list_append(node_data)
            self.role_group_list[role_group].append(self.head)

    def search_role_webpage_data(self, role_group, webpage_name):
        for role, webpage in self.role_group_list.items():
            if role == role_group:
                for i in webpage:
                    if(i.webpage_name == webpage_name):
                        for j in i.data_list:
                            print(f"{j.action_list} + {j.connection_node}")
                            return

    def search_role_data(self, role_group):
        for role, webpage in self.role_group_list.items():
            if role == role_group:
                for i in webpage:
                    for j in i.data_list:
    ...
    def print_role_data(self):
        for role, webpage in self.role_group_list.items():
            print(f"role_group is {role}")
            for i in webpage:
                print(f"Webpage is {i.webpage_name}")
                for j in i.data_list:
                    print(f"{j.action_list} + {j.connection_node}")
            print()

# Example usage
# node_data1 = NodeData("action data1", "connection_data1")
# node_data2 = NodeData("action data2", "connection_data2")
#
# linked_list = LinkedList()
# linked_list.append_node("User", "website_1", node_data1)
# linked_list.append_node("User", "website_2", node_data2)
# linked_list.append_node("SuperUser", "website_1", node_data1)
# linked_list.append_node("SuperUser", "website_2", node_data2)
#
# linked_list.print_role_data()
